...
//        return newArr;
        ...
        int leftLen = point;
        int rightLen = n - point;

        // 왼쪽이 더 짧은 경우
        if (leftLen <= rightLen) {
            // 오른쪽 복사
            int[] result = Arrays.copyOfRange(arr, point, n);

            // 왼쪽을 뒤집어서 더함
            for (int i = 0; i < leftLen; i++) {
                result[i] += arr[point - 1 - i];
            }
        ...
        // 오른쪽이 더 짧은 경우
            ...
            // 왼쪽 복사
            int[] result = Arrays.copyOfRange(arr, 0, point);

            // 오른쪽을 뒤집어서 더함
            for (int i = 0; i < rightLen; i++) {
                result[point - 1 - i] += arr[point + i];
            }
